...
class net(nn.Module):
    def __init__(self, args):
        ...
        self.emb_layer = nn.Embedding(vocab_size, 10)
        
        self.char_conv1=nn.Conv2d(1,32,[7,10])
        self.char_conv2=nn.Conv2d(1,32,[7,10]) 
        self.char_conv3=nn.Conv2d(1,32,[3,10])
        self.char_conv4=nn.Conv2d(1,32,[3,10])
        self.char_conv5=nn.Conv2d(1,32,[3,10]) 
        self.char_conv6=nn.Conv2d(1,32,[3,10]) 
        
        self.char_pool1=nn.MaxPool2d([22+0,1])
        self.char_pool2=nn.MaxPool2d([22+0,1]) 
        self.char_pool3=nn.MaxPool2d([22+4,1]) 
        self.char_pool4=nn.MaxPool2d([22+4,1])
        self.char_pool5=nn.MaxPool2d([22+4,1]) 
        self.char_pool6=nn.MaxPool2d([22+4,1]) 
        ...
        self.word_conv1=nn.Conv2d(1,32,[1,50])
        self.word_conv2=nn.Conv2d(1,32,[1,50]) 
        

        
        self.word_pool1=nn.MaxPool2d(2,2)
        self.word_pool2=nn.MaxPool2d(2,2) 


        # Pass the word Embeddings through the GRU
        self.sent_GRU = nn.GRU(char_dim, hidden_dim, 1, bidirectional=True, batch_first=True)
    ...
    def forward(self, x):                
        ...
        x3=self.char_pool3(self.char_conv3(x)).squeeze().reshape(1200000,32,1)
        x4=self.char_pool4(self.char_conv4(x)).squeeze().reshape(1200000,32,1)
        x5=self.char_pool5(self.char_conv5(x)).squeeze().reshape(1200000,32,1)
        x6=self.char_pool6(self.char_conv6(x)).squeeze().reshape(1200000,32,1)
...
#     iteration=3
    for features, targets in data_iter:
        ...
        probs = network(features).to(device)
...
def train(args, epochs=1, previous_model=False):
    ...
    for epoch_id in range(epochs):
        for i,(x,y) in enumerate(train_dataloader):
            ...
            x = Variable(x.type(torch.long)).to(args.device)
            y = Variable(y.float())#.to(args.device)
...
batch_size = 50# No. of documents in a batch
Nsent=100
Nword=50
Nchar=15
